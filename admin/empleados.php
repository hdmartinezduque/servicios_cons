...
<tbody>';
$add = '../admin/edit_empleado.php';
foreach($data_empleados as $row){
  $edicion = '../admin/edit_empleado.php?id=3&msg='.$mensaje.'&identificacion='.consultasSQL::clean_string(md5($row['cedula']));
  $consulta_empleados .= '<tr>';
  $consulta_empleados .= '<td>'.$row['nombre'].' '.$row['apellido'].'</td>';
  if($editar>0){
    $consulta_empleados .='<td>'.$row['cedula'].'<a href="#" onclick="Eliminar('.$row['id_empleado'].')"><img src="../img/trash.png" height="16" width="16" align = right title="Retirar Empleado"></a></a><a href="'.$edicion.'"> <img src="../img/pen.png" height="16" width="16" align = right title="Click para Editar"></a></td>';
  }else{
...
$head .= '<script type="text/javascript" src="../js/validacion.js"></script>';
$head .= '</head>';
